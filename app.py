...
<style>
    ...
    * { font-family: 'Sarabun', sans-serif; }
    ...
    .metric-card { background: white; padding: 1.5rem; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border-left: 5px solid #667eea; }
    .stButton>button { width: 100%; padding: 0.75rem; font-size: 1.1rem; font-weight: 600; border-radius: 10px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; }
    .success-box { background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); padding: 2rem; border-radius: 15px; color: white; text-align: center; }
    .danger-box { background: linear-gradient(135deg, #eb3349 0%, #f45c43 100%); padding: 2rem; border-radius: 15px; color: white; text-align: center; }
...
@st.cache_data
def load_data():
    return pd.read_csv("https://raw.githubusercontent.com/selva86/datasets/master/Banknote_Authentication.csv")
...
# ================= PAGE 1: หน้าหลัก =================
if page == "🏠 หน้าหลัก":
    st.markdown("""<div class="main-header"><h1>💵 ระบบตรวจจับธนบัตรปลอม</h1><p>Banknote Authentication using Machine Learning & Neural Networks</p></div>""", unsafe_allow_html=True)
    col1, col2, col3, col4 = st.columns(4)
    with col1: st.markdown(f'<div class="metric-card"><h3>📊 ข้อมูลทั้งหมด</h3><h2>{len(df):,}</h2></div>', unsafe_allow_html=True)
...
# ================= PAGE 2: การเตรียมข้อมูล =================
    ...
    st.markdown("""<div class="main-header"><h1>📊 การเตรียมข้อมูล</h1><p>Data Preprocessing & EDA</p></div>""", unsafe_allow_html=True)
    class_counts = df['class'].value_counts()
    col1, col2 = st.columns(2)
    with col1: st.markdown(f'<div class="metric-card"><h3>🟢 ธนบัตรจริง</h3><h2>{class_counts[0]} ({class_counts[0]/len(df)*100:.1f}%)</h2></div>', unsafe_allow_html=True)
    with col2: st.markdown(f'<div class="metric-card"><h3>🔴 ธนบัตรปลอม</h3><h2>{class_counts[1]} ({class_counts[1]/len(df)*100:.1f}%)</h2></div>', unsafe_allow_html=True)
...
# ================= PAGE 3: วิเคราะห์ข้อมูล =================
    ...
    models_info = {
        ...
        "Neural Network (MLP)": "Artificial Neural Network (ANN) แบบ Multi-Layer Perceptron เรียนรู้รูปแบบที่ซับซ้อนผ่าน Hidden Layers"
    }
    ...
    comparison_data = pd.DataFrame({
        ...
        'Accuracy': [0.989, 0.993, 0.996, 0.956, 0.985],
        'Precision': [0.989, 0.993, 0.996, 0.941, 0.982],
        'Recall': [0.989, 0.993, 0.996, 0.957, 0.985],
        'F1-Score': [0.989, 0.993, 0.996, 0.949, 0.983]
    })
...
# ================= PAGE 4: ทายผล =================
    ...
    col1, col2 = st.columns(2)
    with col1:
        variance = st.slider("📊 Variance", -10.0, 10.0, 0.0, 0.1)
        skewness = st.slider("📐 Skewness", -10.0, 10.0, 0.0, 0.1)
    with col2:
        curtosis = st.slider("📏 Curtosis", -10.0, 20.0, 0.0, 0.1)
        entropy = st.slider("🌀 Entropy", -5.0, 5.0, 0.0, 0.1)
    ...
    if st.button("🚀 ทำนายผล (Predict)", use_container_width=True):
        ...
        c5.metric("Neural Network", "ปลอม" if models['Neural Network (MLP)'].predict(scaled_data)[0]==1 else "จริง")
...
# ================= PAGE 5: ผู้พัฒนา =================
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("""<div class="metric-card"><h3>📝 ชื่อโปรเจค</h3><p><b>ระบบตรวจจับธนบัตรปลอม</b><br>Banknote Authentication System</p><h3>🎓 รายวิชา</h3><p>Machine Learning & Deep Learning</p></div>""", unsafe_allow_html=True)
    with col2:
        st.markdown("""<div class="metric-card"><h3>👨‍🎓 ผู้พัฒนา</h3><p>[ใส่ชื่อ-นามสกุล]<br>[รหัสนักศึกษา]</p><h3>📧 อีเมล</h3><p>[your.email@example.com]</p></div>""", unsafe_allow_html=True)
    st.markdown("---")
    ...
    st.markdown("---")
    st.markdown("""<div style="text-align: center; padding: 2rem; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 15px; color: white;"><h2>🙏 ขอบคุณครับ/ค่ะ</h2><p>© 2026 Banknote Fraud Detection System</p></div>""", unsafe_allow_html=True)